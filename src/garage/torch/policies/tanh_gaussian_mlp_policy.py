...
class TanhGaussianMLPPolicy(Policy, TanhGaussianMLPTwoHeadedModule):
    ...
    def __init__(self,
        ...
        TanhGaussianMLPTwoHeadedModule.__init__(
            self,
    ...
    def get_action(self, observation):
        ...
        with torch.no_grad():
            ...
            ret_log_std = (dist.variance**.5).log().squeeze(0).cpu().numpy()
    ...
    def get_actions(self, observations):
        ...
        with torch.no_grad():
            if not isinstance(observations, torch.Tensor):
                observations = tu.from_numpy(observations)
            dist = self.forward(torch.Tensor(observations))
            ret_mean = dist.mean.cpu().numpy()
            ret_log_std = (dist.variance**.5).log().cpu().numpy()
